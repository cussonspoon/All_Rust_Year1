...
fn make_html(documents: &[(String, String)]) -> String {
    ...
    let mut html_table = String::from("<table><tr><th>Document</th><th>Words</th></tr>");

    for (document_name, word_count) in documents {
        html_table.push_str(&format!(
            "<tr><td>{}</td><td>{}</td></tr>",
            document_name, word_count
        ));
    }
...
fn rank_documents(docs: Vec<(String, String)>) -> Vec<(String, String)> {
    ...
    for (doc_name, doc_text) in &docs {
        ...
        temp_result.push((doc_name.clone(), word_count.to_string()));
    }
